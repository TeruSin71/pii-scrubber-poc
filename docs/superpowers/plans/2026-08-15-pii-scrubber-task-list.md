...
> byte-identical to the ORG-fix commit; two checks added by that fix took it
...
**Confirm:** `over_detections 6` — ✅ **confirmed 6 on this machine.** `redacting_spans_emitted 51`. Note the pre-fix run measured 4; the two missing detections were ORG-labelled false positives, restored by the same fix. Also confirmed: `test_fixes.py` **17/17**, `Allowlist loaded: 27 tokens` (correct — the seed literal holds 27; `28` was a doc error, corrected across `Dockerfile`, `FIXES-2026-08-15.md` and this plan). *Count corrected 2026-08-15: the suite emits 16, not 17. Verified byte-identical to the ORG-fix commit; two checks added by that fix took it 14 -> 16. No check was lost.*
...
>
> **REOPENED AND RE-CLOSED 2026-08-16 — 1.2.1 shipped.** Both items landed,
> deployment `d08c99a19640540f`, verified on the endpoint rather than inferred
> from the artifact. The result worth carrying from this one is not a number —
> 1.2.1 produced none, and could not, because all four evaluation sets are
> burned. It is that **the identity check that would have failed in production
> passed locally**: `/info` is not proxied by the AI Core gateway, so a stamp
> reading it alone was green on every local run and would have reported
> `unreachable` forever on the deployment it was built for. Caught in review,
...
|---|---|---|---|
| 2026-08-16 | **1.2.3 close-out** | ✅ | **Shipped, cut over, verified end to end — and the release's real product was a retraction.** Deployment **`daedcfe9342d21a7`** (replaces `da1b1b3e39367c59`), image `1.2.3` `linux/amd64` `sha256:9fcc4337…58d080735`. Deployed `/v1/info` → `1.2.3`; `/v1/selftest` → `1.2.3 / 100.0 / 45/45 / missed 0 / over_detections 4 / spans 49`, **identical to the container and to local**. Gates exact on the deployment: `108/111`, `65/68`, `45/50`, leak lists line-by-line, each run self-stamped `build: 1.2.3`. Suites 16 / 39 / 33 / 74 / 29 / 37. **Item 3 (`FAC` → `ADDRESS`) was WITHDRAWN mid-release as a measured no-op:** `FAC` never reaches `LABEL_MAP` — `SpacyRecognizer` does not declare the entity, so the span dies at the recognizer and `detect()` returns `[]` both before and after the mapping. The 4-then-8-span incidence probe that justified it had been measured with **raw spaCy, not through the pipeline** — a true measurement at the wrong layer, now a named trap class. The release became the corrections that wrong mechanism had left behind: the 1.2.2 log line, the handover's `_norm()` claim, and `unmapped_labels()`'s semantics (**a superset of the leak list, not the leak list**). Review then found three more: the payload shipped a bare `["FAC"]` with the caveat nowhere a reader would look (**fixed — semantics now ship in the payload**); no test could fail if the corrections were reverted (**fixed — pinned, and proved by mutation, each revert producing a red suite**); and §5's suite registration had been stale since `24eaef3` (**fixed — re-pinned, plus a standing rule that a registered number is re-pinned in the commit that moves it**). Also registered for the recognizer plan: the nine `supported_entities` are exactly the distinct values of `model_to_presidio_entity_mapping`; the guard is **two-condition** (supported AND requested); and the cited ORG precedent is **dormant** — that branch never fires at this pin, so a `FAC` change would be the first time it ever did. **No quotable figure, and none was possible.** The unnumbered-street leak class is real, measured, and still open at the recognizer layer |
